...
const useRadialMenu = (
	...
	useEffect(() => {
		...
		const handleActivationClick = (e: MouseEvent) => {
		...
		window.addEventListener('mouseup', handleActivationClick, true)
		return () => window.removeEventListener('mouseup', handleActivationClick, true)
	}, [enabled, activationButton])
