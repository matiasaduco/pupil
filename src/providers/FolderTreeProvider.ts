...
export class VsCodeFolderNode extends vscode.TreeItem {
	constructor(public readonly fullPath: string) {
		super(path.basename(fullPath), vscode.TreeItemCollapsibleState.Collapsed)
...
export class FolderTreeProvider implements vscode.TreeDataProvider<VsCodeFolderNode> {
	private _onDidChangeTreeData: vscode.EventEmitter<VsCodeFolderNode | undefined | void> =
		new vscode.EventEmitter()
	readonly onDidChangeTreeData: vscode.Event<VsCodeFolderNode | undefined | void> =
	...
	getChildren(element?: VsCodeFolderNode): Thenable<VsCodeFolderNode[]> {
		const root = element ? element.fullPath : vscode.workspace.workspaceFolders?.[0].uri.fsPath
		...
		const dirs = fs
			...
			.filter(
				(p) => fs.statSync(p).isDirectory() && !p.includes('node_modules') && !p.includes('public')
			)
